...
import { useSize } from "../../Core/shared";

import { BaseSliderThumbImplProps } from "./types";

const props = defineProps<BaseSliderThumbImplProps>();
...
const percent = computed(() =>
  value.value === undefined
    ...
    : convertValueToPercentage(
        value.value,
        context?.min.value ?? 0,
        context?.max.value ?? 100
      )
);
const label = computed(() =>
  getLabel(props.index, context?.modelValue?.value?.length ?? 0)
);
...
//  custom hook για το isMounted
const useCustomMounted = () => {
...
const isMounted = useCustomMounted();
...
onUnmounted(() => {
  const i =
    context?.thumbElements.value.findIndex((i) => i === thumbElement.value) ??
    -1;
